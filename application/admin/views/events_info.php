...
<div class="form-group">
    ...
    <h5 id="title_text"><?php echo $title; ?></h5>
    <input type="text" name="title" id="title" style="display: none" value="<?php echo $title; ?>" class="form-control"/>
</div>

<div class="form-group">
    <label class="control-label">Description</label>
    <h5 id="description_text"><?php echo $description; ?></h5>
    <input type="text" name="description" id="description" style="display: none" value="<?php echo $description; ?>" class="form-control"/>
</div>
...
<div class="form-group">
    ...
    <h5 id="start_text">
        ...
        if($allDay){
            ...
            echo date('Y-m-d H:i', strtotime($start));
        }
        ?>
    </h5>


    <p id="start_p" style="display: none">
...
<div id="p_endtime" class="form-group" <?php echo $end_display; ?>>
    ...
    <h5 id="end_text">
        ...
        if($allDay){
            ...
            echo date('Y-m-d H:i', strtotime($end));
        }
        ?>
        <br/>
        <br/>
...
<div class="form-group" style="display: none" id="option">
    ...
    <div>
        <label><input type="checkbox" value="1" id="isallday" name="isallday" <?php echo $allday_chk; ?>> All
            Day</label>
...
<div class="form-group" style="display: none" id="control">
    <div class="controls">
        <button id="add-new-event" type="submit" class="btn btn-primary"
                style="border-color: <?php echo $backgroundColor; ?>; background-color: <?php echo $backgroundColor; ?>;">
            Save
        </button>
        <button type="button" class="btn btn-default" onclick="toggleEdit()">cancel</button>
        <?php
        if ($id != '') {
            ?>
            <button type="button" class="btn btn-danger"
                    onclick="del_confirm('Notice', 'Are you sure？', '<?php echo site_url('c=calendar&m=delete&id=' . $id); ?>')">
                delete
            </button>
            <?php
        }
...
<div class="form-group" id="review">
    <button id="participate-btn" class="btn btn-warning" onclick="toggleParticipate()">
        Participate
    </button>

    <button id="edit-btn" class="btn btn-primary" <?php echo $edit_display?> onclick="toggleEdit()">
        Edit
    </button>
</div>
...
<script>
    ...
    function toggleParticipate() {
        $.ajax({
            type: "POST",  //提交方式
            url: "admin.php?c=calendar&m=participate",//路径
            data: "id=<?php echo $id?>",
            success: function (result) {//返回数据根据结果进行相应的处理
                if (result) {
                    var json = $.parseJSON(result);
                    if(json.success){
                        if ($("#participate-btn").hasClass("btn-warning")) {
                            $("#participate-btn").removeClass("btn-warning").addClass("btn-success").html("Participated");
                        } else {
                            $("#participate-btn").removeClass("btn-success").addClass("btn-warning").html("Participate");
                        }
                    }
                    return false;
                }
    ...
    function checkIfParticipated() {
        $.ajax({
            type: "POST",  //提交方式
            url: "admin.php?c=calendar&m=ifParticipate",//路径
            data: "id=<?php echo $id?>",
            success: function (result) {//返回数据根据结果进行相应的处理
                if (result) {
                    $("#participate-btn").removeClass("btn-warning").addClass("btn-success").html("Participated");
...
<script type="text/javascript"
        src="<?php echo base_url(); ?>resource/adminlte/plugins/datepicker/bootstrap-datepicker.js"></script>
<script type="text/javascript">
    $(function () {
