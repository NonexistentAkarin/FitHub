...
CREATE INDEX `idx_userid` ON `events`(`user_id`);
...
CREATE TABLE `permissions` (
  `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, -- '编号',
  `permKey` varchar(255) NOT NULL, -- '权限key',
  `permName` varchar(255) NOT NULL, -- '权限名称',
  `parent_id` unsigned int(11) NOT NULL DEFAULT '0', -- '上级权限编号',
  `lft` int(11) DEFAULT '0',
  `rgt` int(11) DEFAULT '0',
  `root_id` unsigned int(11) NOT NULL DEFAULT '0',
  `addtime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- '添加时间',
  `sortid` int(11) NOT NULL DEFAULT '0', -- '排序',
  `issys` int(11) NOT NULL DEFAULT '0', -- '系统的',
  `permType` int(11) DEFAULT '0', -- '权限类型',
  `rel_id` int(11) DEFAULT '0' -- '权限对应类型对应记录的id',
); -- ='权限表';

CREATE UNIQUE INDEX `permKey` ON `permissions` (`permKey`);
CREATE INDEX `parent_id` ON `permissions`(`parent_id`);
CREATE INDEX `lft` ON `permissions`(`lft`);
CREATE INDEX `rgt` ON `permissions`(`rgt`);
CREATE INDEX `root_id` ON `permissions`(`root_id`);
CREATE INDEX `permissions_issys` ON `permissions`(`issys`);
...
CREATE TABLE `roles` (
  `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, -- '角色编号',
  `roleName` varchar(20) NOT NULL, -- '角色名称',
  `issys` int(11) NOT NULL DEFAULT '0', -- '系统的',
  `company_id` unsigned int(11) DEFAULT '0'
); -- ='角色表';

CREATE INDEX `roles_issys` ON `roles`(`issys`);
CREATE INDEX `roleName` ON `roles`(`roleName`);
...
CREATE TABLE `role_perms` (
  `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, -- '编号',
  `roleID` bigint(20) NOT NULL DEFAULT '0', -- '角色编号',
  `permID` bigint(20) NOT NULL DEFAULT '0', -- '权限编号',
  `value` tinyint(1) NOT NULL DEFAULT '0', -- '值',
  `addDate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP -- '添加时间',
); -- ='角色权限表';

CREATE UNIQUE INDEX `roleID_2` ON `role_perms`(`roleID`,`permID`);
...
CREATE TABLE `users` (
  `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, -- '用户编号',
  `username` varchar(255) NOT NULL, -- '登录名',
  `password` varchar(255) NOT NULL, -- '密码',
  `email` varchar(255) NOT NULL, -- '邮箱',
  `cur_login_time` datetime DEFAULT NULL, -- '当前登录时间',
  `cur_login_ip` varchar(255) DEFAULT NULL, -- '当前登录ip',
  `cur_login_area` varchar(255) DEFAULT NULL, -- '当前登录地区',
  `last_login_ip` varchar(255) DEFAULT NULL, -- '最后登录ip',
  `last_login_area` varchar(255) DEFAULT NULL, -- '最后登录地区',
  `last_login_time` datetime DEFAULT NULL, -- '最后登录时间',
  `reg_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- '添加时间',
  `reg_ip` varchar(255) DEFAULT NULL, -- '注册ip',
  `reg_area` varchar(255) DEFAULT NULL, -- '注册地区',
  `status` int(11) NOT NULL DEFAULT '1', -- '状态',
  `login_times` int(11) NOT NULL DEFAULT '0', -- '登录次数',
  `owner_sites` text,
  `parent_user_id` unsigned int(11) NOT NULL DEFAULT '0',
  `company_id` unsigned int(11) NOT NULL DEFAULT '0',
  `photo` varchar(255) DEFAULT NULL,
  `user_type` unsigned tinyint(1) DEFAULT '0',
...
CREATE TABLE `user_perms` (
  `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, -- '编号',
  `userID` unsigned bigint(20) NOT NULL DEFAULT '0', -- '用户编号',
  `permID` bigint(20) NOT NULL DEFAULT '0', -- '权限编号',
  `value` tinyint(1) NOT NULL DEFAULT '0', -- '值',
  `addDate` datetime NOT NULL -- '添加时间',
); -- ='用户权限表';

CREATE UNIQUE INDEX `user_permsID` ON `user_perms`(`userID`,`permID`);
...
CREATE TABLE `user_roles` (
  `userID` unsigned int(11) NOT NULL DEFAULT '0', -- '用户编号',
  `roleID` bigint(20) NOT NULL DEFAULT '0', -- '角色编号',
  `addDate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- '添加时间',
  `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL -- '编号',
); -- ='用户角色表';

CREATE UNIQUE INDEX `user_rolesID` ON `user_roles`(`userID`,`roleID`);
